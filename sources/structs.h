...
#ifdef WINDOWS
    UBYTE d_u_m_m_y[__alignof(off_t) - (( sizeof(long) * (1) + sizeof(int) * (5) + sizeof(unsigned char) * (16 + 1)) & (__alignof(off_t) - 1))];
#else
...
#ifdef WINDOWS
    SBYTE	empty[(512 - 2 * 8 - ((512 - 2 * 8) / sizeof(INDEXENTRY)) * sizeof(INDEXENTRY))];		/**< Padding to 512 bytes */
#else
